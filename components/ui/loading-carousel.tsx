...
export function LoadingCarousel({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "w-full max-w-6xl mx-auto rounded-lg bg-transparent shadow-[0px_1px_1px_0px_rgba(0,0,0,0.05),0px_1px_1px_0px_rgba(255,252,240,0.5)_inset,0px_0px_0px_1px_hsla(0,0%,100%,0.1)_inset,0px_0px_1px_0px_rgba(28,27,26,0.5)]",
        className
      )}
    >
      <div className="w-full overflow-hidden rounded-lg">
        ...
        >
          ...
          {showNavigation && (
            <>
              <CarouselPrevious className="absolute left-2 top-1/2 text-neutral-500 -translate-y-1/2" />
              <CarouselNext className="absolute right-2 text-neutral-500 top-1/2 -translate-y-1/2" />
            </>
          )}
        </Carousel>
        <div
        ...
        >
          ...
          >
            {showIndicators && (
              <div className="flex space-x-2 overflow-x-auto pb-2 sm:pb-0 justify-center sm:justify-start w-full sm:w-auto">
                {(displayTips || []).map((_, index) => (
                  <motion.button
                    key={index}
                    className={`h-1 w-2 sm:w-8 flex-shrink-0 rounded-full ${index === current ? "bg-muted" : "bg-primary"
                      }`}
                    initial={false}
                    animate={{
                      backgroundColor:
            ...
            <div className="flex items-center space-x-2 text-emerald-500 whitespace-nowrap">
              {backgroundTips ? (
                ...
                <div className="flex flex-col">
                  {displayTips[current]?.url ? (
                    ...
                    <span className="text-center min-h-20 sm:min-h-4 w-full text-base lg:text-2xl xl:font-semibold tracking-tight break-words whitespace-normal font-medium leading-snug">
                      {animateText ? (
                        <TextScramble
                          key={displayTips[current]?.text}
                          duration={1.2}
